...
try:
	for epoch in range(numEpochs):
		...
		for batch in range(numBatches):
			...
			Pw_cost = Agent.mentor(state_batch, Pw_batch)
			Pw_cost_sum += Pw_cost
			run_time = time.clock()-t
			iteration+=1
			#print("Time per position: "+str(run_time/batch_size)+" Pw Cost: "+str(Pw_cost))
